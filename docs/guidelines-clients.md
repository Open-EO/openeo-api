...
* **methodName** in `hasFeature`: The name of a client method in API scope.
...
| Update a stored process graph.                    | `PATCH /process_graphs/{process_graph_id}`  | `updateProcessGraph(?processGraph, ?title, ?description)` |
